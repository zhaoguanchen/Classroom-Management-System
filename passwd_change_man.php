 ...
 if($name == "" || $passwd == "" || $confirm == "" || $repasswd == "" )  
   ...
   //       $class = $_POST["class"];
        ...
        /*根据用户通过POST提交的数据组合插入数据库的SQL语句*/  
            ...
            $sql_12 = "UPDATE `manager` SET `password`=$repasswd WHERE name = '$name'";  
...
// $log->addlog ( "./log", "a", " 内容1:$content1  内容2: $content2  内容3: $content3 " ); //传多个内容  
 $log->addlog ( "./logs/log_man_changepasswd", "a", "管理员$name  修改密码  日期 $date  时间$time" ); //一次插入并换行        
$log->addlog ( "./logs/log_man_changepasswd", "a", "\n" ); //一次插入并换行  
