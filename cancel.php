...
?>
  ...
  我的借用（有效）<br>
 <form action="cancel_1.php" method="post"> 
     <br /><br /> 
     请选择要取消的类型： <br/><br/>
	<select name="type">
...
<tr cellspacing=10 height=20 bgcolor="#FF00CC"><th>记录ID</th><th>教室ID</th><th>教室名</th><th>位置</th><th>座位数</th><th>借用日期</th><th>借用时间</th><th>事由</th><th>操作时间</th></tr>
    <?php
      $flag;
	    $nowdate= date("Y-m-d");
         include "./includes/mysql_connect.php"; 
		 $sql = "  SELECT * FROM `classborrow`  WHERE  `pname` = $name and  `date` >= '$nowdate' "; //and  `date` <= $nowdate  and datediff(day,$date,$nowdate)<=0
   ...
   if($num)
   {
          for($i=0;$i<$num;$i++){
		        ...
		        $max = $sql_arr_1['max'];
            echo "<tr bordercolor='#66CCCC' align='center'  bgcolor='#99FF00' ><td>$sid</td><td>$id</td><td>$cname</td><td>$location</td><td>$max</td><td>$date</td><td>$time</td><td>$reason</td><td>$operatetime</td></tr>";
               }
   }
   else
   { 
...
<br/>
 ...
 <div align="center">
<table border=10 cellspacing=10  align="center" bordcolor = "#00FF00" width="800"  height="100" bgcolor="#FFFF99"> 
<tr cellspacing=10 height=20 bgcolor="#FF00CC"><th>记录ID</th><th>实验室ID</th><th>实验室名</th><th>位置</th><th>座位数</th><th>借用日期</th><th>借用时间</th><th>事由</th><th>操作时间</th></tr>
    <?php
      $flag;
	    $nowdate= date("Y-m-d");
         include "./includes/mysql_connect.php"; 
		 $sql = "  SELECT * FROM `labborrow`  WHERE  `pname` = $name and  `date` >= '$nowdate' "; //and  `date` <= $nowdate  and datediff(day,$date,$nowdate)<=0
		 $result=mysql_query($sql);  
		 $num = mysql_num_rows($result); //统计执行结果影响的行数  
   ...
   if($num)
   {
          for($i=0;$i<$num;$i++){
				...
				$operatetime = $sql_arr['operatetime'];
				//教室信息
				$sql_1 = "  SELECT * FROM `lab`  WHERE  `ID` =$id"; 
				$result_1=mysql_query($sql_1);  
			    $num_1 = mysql_num_rows($result_1); //统计执行结果影响的行数  
				$sql_arr_1 = mysql_fetch_assoc($result_1);
                $cname = $sql_arr_1['name'];
 ...
 <div align="center">
<table border=10 cellspacing=10  align="center" bordcolor = "#00FF00" width="800"  height="100" bgcolor="#FFFF99"> 
<tr cellspacing=10 height=20 bgcolor="#FF00CC"><th>记录ID</th><th>会议室ID</th><th>会议室名</th><th>位置</th><th>座位数</th><th>借用日期</th><th>借用时间</th><th>事由</th><th>操作时间</th></tr>
    <?php
      $flag;
	    $nowdate= date("Y-m-d");
         include "./includes/mysql_connect.php"; 
		 $sql = "  SELECT * FROM `meetborrow`  WHERE  `pname` = $name and  `date` >= '$nowdate' "; //and  `date` <= $nowdate  and datediff(day,$date,$nowdate)<=0
		 $result=mysql_query($sql);  
		 $num = mysql_num_rows($result); //统计执行结果影响的行数  
   ...
   if($num)
   {
          for($i=0;$i<$num;$i++){
				...
				$operatetime = $sql_arr['operatetime'];
				//教室信息
				$sql_1 = "  SELECT * FROM `meeting`  WHERE  `ID` =$id"; 
				$result_1=mysql_query($sql_1);  
			    $num_1 = mysql_num_rows($result_1); //统计执行结果影响的行数  
				$sql_arr_1 = mysql_fetch_assoc($result_1);
                $cname = $sql_arr_1['name'];
