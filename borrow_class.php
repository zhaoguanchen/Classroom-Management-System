...
<form action="borrow_class.php" method="post"> 
请选择借用日期：<br/>
 ...
 请选择要借用的时间段（仅选一个）： <br/><br/>
   ...
   <input type="submit" name="submit"style="height:30px;width:60px;display:inline-block;"    value="查询">
     ...
     <?php
	  ...
	  ?>
<form action="borrow_class_2.php" method="post"> 
     <br /><br /> 
...
<table border=10 cellspacing=10  bordcolor = "#00FF00" width="800"  height="100" bgcolor="#FFFF99"> 
    ...
    <?php
      $flag;
        ...
        //查询数据表中的数据 "SELECT * FROM `classborrow` WHERE `name` = $name and `number` = $num ";
		
	//	if($x1==1 && $x2==2 && $x3==3 && $x4==4 && $x5==5 )	{ $sql = "SELECT * FROM `classborrow` WHERE `fir`=1 and `sec`=1 and `thi`=1  and`fou`=1 and`fif`=1 and`six`=1   ";  }
	//	 if($x1==1 && $x2==2 && empty($x1) && empty($x1)&&empty($x1))	{ $sql = "SELECT * FROM `classborrow` WHERE `fir`=1 and `sec`=1 and `thi`=1  and`fou`=1 and`fif`=1 and`six`=1   ";  }
		// if($x1==1 && $x2==2 && $x3==3 && $x4==4 && $x5==5 )	{ $sql = "SELECT * FROM `classborrow` WHERE `fir`=1 and `sec`=1 and `thi`=1  and`fou`=1 and`fif`=1 and`six`=1   ";  }
	//	 if($x1==1 && $x2==2 && $x3==3 && $x4==4 && $x5==5 )	{ $sql = "SELECT * FROM `classborrow` WHERE `fir`=1 and `sec`=1 and `thi`=1  and`fou`=1 and`fif`=1 and`six`=1   ";   }
        
		//最多选两个时间段  WHERE classborrow.date=$date and classborrow.time=$time 
	...
	//	where id not in 
	 ...
	 //    $sql_1 = "SELECT ID FROM `classroom`  WHERE 1 "; 
