...
?>
  ...
  我的预定（有效）<br>
  <?php echo '点击此处 <a href="cancel_long.php">取消预约</a> ' ;               
     ?>                   
    <br/>             
...
<div align="center">
<table border=10 cellspacing=10 align="center" bordcolor = "#00FF00" width="800"  height="100" bgcolor="#FFFF99"> 
<tr cellspacing=10 height=20 bgcolor="#FF00CC"><th>教室ID</th><th>教室名</th><th>位置</th><th>座位数</th><th>借用日期</th><th>开始时间</th><th>结束时间</th><th>事由</th><th>操作时间</th></tr>
    <?php
      $flag;
	    $nowdate= date("Y-m-d");
         include "./includes/mysql_connect.php"; 
   ...
   if($num)
   {
          for($i=0;$i<$num;$i++){
				...
				$location = $sql_arr_1['location'];
		        $max = $sql_arr_1['max'];
            echo "<tr bordercolor='#66CCCC' align='center'  bgcolor='#99FF00' ><td>$id</td><td>$cname</td><td>$location</td><td>$max</td><td>$date</td><td>$startdate</td><td>$enddate</td><td>$reason</td><td>$operatetime</td></tr>";
               }
   }
   else
   { 
