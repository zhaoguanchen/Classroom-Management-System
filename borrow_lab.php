...
<div id="left">
<form action="borrow_lab.php" method="post"> 
请选择借用日期：<br/>
 ...
 请选择要借用的时间段（仅选一个）： <br/><br/>
   ...
   <input type="submit" name="submit"style="height:30px;width:60px;display:inline-block;"    value="查询">
     ...
     <?php
	  ...
	  ?>
<form action="borrow_lab_2.php" method="post"> 
     <br /><br /> 
...
<table border=10 cellspacing=10  bordcolor = "#00FF00" width="800"  height="100" bgcolor="#FFFF99"> 
         <tr cellspacing=10 height=20 bgcolor="#FF00CC"><th>实验室ID</th><th>实验室名</th><th>位置</th><th>座位数</th></tr>
    <?php
         include "./includes/mysql_connect.php";  
		 $sql = "  SELECT * FROM `lab`  WHERE ID not in ( SELECT cid FROM `labborrow`  WHERE `date` = '$date' and `time` = '$time')"; 
